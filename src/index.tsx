...
async function generateAvatarGridImage(width: number, height: number, avatarUrls: string[], iconSize: number, gap: number) {
...
app.get("/:orgslug", async c => {
  ...
  const width = widthParam > 0
    ? widthParam
    : gap + columns * iconSize + gap * (columns - 1) + gap
  const height = heightParam > 0
    ? heightParam
  ...
  const img = await generateAvatarGridImage(width, height, avatarsBase64, iconSize, gap)
